...
public class PDFGenerator {
    ...
    public static void bulletinGenerator(Classe classe, List<Module> modules, int semestre) throws PDFException {
        ...
        List<Apprenant> apprenantsPDF = new ArrayList<>(classe.getApprenants());
        List<Module> modulesPDF = new ArrayList<>(modules);

        // Supprimer les modules qui ne sont pas du bon semestre
        modulesPDF.removeIf(module -> module.getSemestre() != semestre);


        // Verifier si la liste des eleves est vide
        if (apprenantsPDF.isEmpty()) {
        ...
        apprenantsPDF.removeIf(apprenant -> apprenant.getEtatPaiement() == 0);

        // Recuperer la liste des notes
        List<Note> notes = new ArrayList<>();
        for (Module module : modulesPDF) {
            notes.addAll(module.getNotes());
        }

        for (Apprenant apprenant : apprenantsPDF) {
            ...
            try {
                ...
                tableLineGenerator(table, modulesPDF, apprenantNotes);
